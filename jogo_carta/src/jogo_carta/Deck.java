...
public class Deck {
	...
	public Deck() {
		for (int i = 0; i < 10; i++) {
			Carta carta = new Carta(0, 10); // DISTANCIA DAS CARTAS
			carta.score =+1;
			carta.sprite = Spritesheet.getSprite(i * 71, 0, 71, 95); //DIMENSAO
			cartas.add(carta);
		}
		 ...
		 * 
		 * @Override public int compare(Carta z1, Carta z2) { if (z1.x > z2.x) return 1;
		 * if (z1.x < z2.x) return -1; return 0; } });
		 */
	...
	public void render(Graphics g) { // LOOPING NAS CARTAS
		for (int i = 0; i < cartas.size(); i++) {
